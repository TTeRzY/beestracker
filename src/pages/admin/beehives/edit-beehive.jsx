...
import {GET_BEEHIVE, UPDATE_BEEHIVE} from "../../../graphql/beehives.js";
import {useSelector} from "react-redux";

export default function EditBeehive() {
    const navigate = useNavigate()
    const {id} = useParams()
    const apiaries = useSelector(state => state.apiaries)
    const {loading: beehiveLoading, error: beehiveError, data: beehiveData} = useQuery(GET_BEEHIVE, {
        variables: {
            _id: id
        }
    })
    const [updateBeehive, {loading, error}] = useMutation(UPDATE_BEEHIVE)

    const validationSchema = Yup.object().shape({
        beehiveId: Yup.string()
            .min(1, "Минималната дължина е 3 символа")
            .required("Това поле е задължително"),
        apiary: Yup.string()
            .min(4, "Минималната дължина е 3 символа")
            .required("Това поле е задължително"),
        beehiveType: Yup.string()
            .min(6, "Минималната дължина е 3 символа")
            .required("Това поле е задължително"),
        ...
        <div className="add-beehive-page px-4 pt-6 flex items-center justify-center">
            <div className="add-beehive-form-wrapper w-full xl:w-5/6 shadow-lg border-gray-200 rounded-lg bg-white">
                ...
                <div className="add-beehive-wrapper p-5 xl:flex xl:items-center xl:justify-center shadow-b-lg">
                    {!beehiveLoading && <Formik
                        initialValues={
                            {
                                beehiveId: beehiveData?.beehive.beehiveId ?? "",
                                apiary: beehiveData?.beehive.apiary ?? "",
                                beehiveType: beehiveData?.beehive.beehiveType ?? "",
                                familyType: beehiveData?.beehive.familyType ?? ""
                            }
                        }
                        ...
                        values={{
                            beehiveId: beehiveData?.beehive.beehiveId ?? "",
                            apiary: beehiveData?.beehive.apiary._id ?? "",
                            beehiveType: beehiveData?.beehive.beehiveType ?? "",
                            familyType: beehiveData?.beehive.familyType ?? ""
                        }}
                    >
                            ...
                            <Form className="space-y-4 md:space-y-6 xl:w-5/6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        onBlur={handleBlur}
                                        value={beehiveData.beehive.apiary._id}
                                        id="apiary"
                                        name="apiary"
