...
import {CREATE_APIARY, GET_APIARY, UPDATE_APIARY} from "../../../graphql/apiaries.js";
...
export default function EditApiary() {
    ...
    const {id} = useParams()
    const {loading: apiaryLoading, error: apiaryError, data: apiaryData} = useQuery(GET_APIARY, {
        variables: {
            _id: id
        }
    })
    const [updateApiary, {loading, error}] = useMutation(UPDATE_APIARY)
            ...
            <div className="add-apiary-form-wrapper xl:w-5/6 shadow-sm border-gray-200 rounded-lg bg-white">
                ...
                <div className="add-apiary-wrapper p-5 flex items-center justify-center">
                    {!apiaryLoading && <Formik
                        initialValues={
                            {
                                apiaryId: apiaryData?.apiary.apiaryId ?? "",
                                apiaryType: apiaryData?.apiary.apiaryType ?? "",
                                location: apiaryData?.apiary.location ?? "",
                            }
                        }
                        ...
                        {({
                            ...
                            <Form className="space-y-4 md:space-y-6 xl:w-5/6">
                                <FormInput
                                    ...
                                    error={errors.apiaryId && touched.apiaryId}
                                />
                                <div>
                                    <label
                                        htmlFor="apiaryType"
                                    ...
                                    <select
                                        onChange={handleChange}
                                        onBlur={handleBlur}
                                        value={apiaryData?.apiary.apiaryType}
                                        id="apiaryType"
                                        name="apiaryType"
